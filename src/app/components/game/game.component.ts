    ...
    <div class="app-container">
      <div class="card" style="text-align: center;">
        ...
        <div *ngIf="gameService.gameState() === 'PLAYING'">
          <button
            class="btn"
            style="background: linear-gradient(45deg, #ff4081, #ff80ab);"
            (click)="revealImpostor()"
          >
            DESCUBRIR IMPOSTOR
          </button>
        </div>
        ...
        <div *ngIf="gameService.gameState() === 'FINISHED'">
          ...
          <button class="btn" (click)="nextRound()" [disabled]="gameService.isLoading()">
            {{ gameService.isLoading() ? 'CARGANDO...' : 'SIGUIENTE RONDA' }}
          </button>
          <button class="btn btn-secondary" (click)="exit()" style="margin-top: 10px;">
            SALIR
          </button>
        </div>
      </div>
